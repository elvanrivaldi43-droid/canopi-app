...
| 11 | Label S{n} di kanvas dipisah dari grid (diputuskan 21 Agustus 2026) | Sekarang label "S{n}" di kanvas dihitung gabungan grid+manual dalam 1 urutan (`render()` ~baris 1125-1130, `mem.filter(jenis==='support').forEach((m,i)=>...)`) — tidak sama dengan nomor S1/S2 yang akan ditampilkan panel (yang dihitung dari `S.supportsManual` saja). **Dipisah:** support manual dapat nomor S{n} dari posisinya di `S.supportsManual` — SAMA PERSIS di kanvas maupun di panel, tidak berubah walau grid berubah. Support grid otomatis TIDAK diberi nomor S lagi di kanvas (cukup material·panjang tanpa label nomor) — konsisten dengan alasan Keputusan #5 (grid tidak addressable/tidak stabil). |
