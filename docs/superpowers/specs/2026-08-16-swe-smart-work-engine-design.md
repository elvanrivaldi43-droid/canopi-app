...
| 5b | Skill karyawan — deteksi tukang/kenek dari jabatan (diputuskan 21 Agustus 2026) | `users.jabatan` adalah **teks bebas** (contoh nyata: "Tukang Las", "Admin Sales", "Surveyor" — `karyawan/edit.blade.php:120`), bukan enum. **Tidak ada field baru ditambahkan.** Deteksi pakai keyword match case-insensitive: mengandung "kenek" → kategori Kenek; mengandung "tukang" → kategori Tukang (termasuk "Kepala Tukang"). Kalau jabatan tidak mengandung kata manapun → skill default TIDAK nempel otomatis, DAN halaman edit Karyawan menampilkan badge peringatan eksplisit ("Kategori lapangan tidak terdeteksi dari jabatan — centang skill manual") supaya kekosongan ini kelihatan, bukan diam-diam kosong. |
...
**Koreksi dari asumsi brainstorming awal (diverifikasi ke kode 21 Agustus 2026):** `rab_jenis_kerja.skill_default` **sudah** berupa dropdown yang keisi dari `rab_skill.nama` (`resources/views/produktivitas/index.blade.php:116-118`, `ProduktivitasController.php:72`), bukan teks bebas — jadi item "ubah jadi dropdown" DIHAPUS dari scope Fase 2, sudah beres duluan. Yang masih perlu Fase 2 kerjakan cuma matching-nya di backend: `user_skill.rab_skill_id` dicocokkan ke skill yang namanya sama persis dengan `rab_jenis_kerja.skill_default` (exact match by nama, karena dropdown sudah menjamin nilainya valid — tidak perlu case-insensitive/fuzzy matching).
...
**Batas scope Fase 2 khusus** (disetujui Elvan 21 Agustus 2026): Fase 2 cuma mengerjakan `rab_skill.default_role`, tabel `user_skill`, checklist skill di halaman Karyawan, matching `skill_default` di backend (dropdown-nya sendiri sudah ada, lihat koreksi di Bagian 4-B), dan tombol+endpoint "Hitung Saran" di panel Mulai Tahap. TIDAK termasuk dashboard `/kapasitas-tim`, notif Telegram defisit skill, maupun `evaluasi_produktivitas` — itu Fase 3 dan Fase 4, dikerjakan terpisah setelah Fase 2 selesai & diuji.
