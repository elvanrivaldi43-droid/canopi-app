...
<div style="max-width:600px;margin:0 auto;">
    ...
    <div style="display:flex;align-items:center;gap:12px;margin-bottom:20px;">
        <a href="{{ route('pipeline.show', $lead) }}" style="display:inline-flex;align-items:center;justify-content:center;width:36px;height:36px;background:#1e293b;border-radius:10px;text-decoration:none;border:1px solid #334155;flex-shrink:0;">
        ...
        <div>
            <div style="font-size:20px;font-weight:800;color:#f1f5f9;">Edit Lead</div>
            <div style="font-size:12px;color:#64748b;">{{ $lead->nama_customer }}</div>
        </div>
    </div>

    <form method="POST" action="{{ route('pipeline.update', $lead) }}" x-data="{ status: '{{ old('status', $lead->status) }}' }">
        @csrf @method('PUT')
        ...
        <div style="background:#1e293b;border-radius:16px;padding:18px;margin-bottom:14px;border:1px solid #334155;">
            ...
            <div style="margin-bottom:14px;">
                ...
                <input type="text" name="nama_customer" value="{{ old('nama_customer',$lead->nama_customer) }}" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid {{ $errors->has('nama_customer') ? '#ef4444' : '#334155' }};border-radius:10px;padding:10px 14px;font-size:14px;box-sizing:border-box;" required>
            ...
            <div style="margin-bottom:14px;">
                ...
                <input type="tel" name="no_hp" value="{{ old('no_hp',$lead->no_hp) }}" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid {{ $errors->has('no_hp') ? '#ef4444' : '#334155' }};border-radius:10px;padding:10px 14px;font-size:14px;box-sizing:border-box;" required>
            ...
            <div>
                ...
                <textarea name="alamat" rows="2" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid #334155;border-radius:10px;padding:10px 14px;font-size:14px;resize:vertical;box-sizing:border-box;">{{ old('alamat',$lead->alamat) }}</textarea>
            </div>
        </div>
        ...
        <div style="background:#1e293b;border-radius:16px;padding:18px;margin-bottom:14px;border:1px solid #334155;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:14px;margin-bottom:14px;">
                <div>
                    ...
                    <select name="produk" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid #334155;border-radius:10px;padding:10px 12px;font-size:14px;" required>
                        @foreach($produkList as $p)
                        <option value="{{ $p }}" {{ old('produk',$lead->produk)==$p ? 'selected' : '' }}>{{ ucfirst($p) }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    <label style="display:block;font-size:12px;font-weight:600;color:#94a3b8;margin-bottom:6px;">Sumber Lead *</label>
                    <select name="sumber_lead" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid #334155;border-radius:10px;padding:10px 12px;font-size:14px;" required>
                        @foreach($sumberList as $s)
                        <option value="{{ $s }}" {{ old('sumber_lead',$lead->sumber_lead)==$s ? 'selected' : '' }}>{{ $s }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
            ...
            <div>
                ...
                <input type="number" name="estimasi_nilai" value="{{ old('estimasi_nilai',$lead->estimasi_nilai) }}" min="0" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid #334155;border-radius:10px;padding:10px 14px;font-size:14px;box-sizing:border-box;">
            </div>
        </div>
        ...
        <div style="background:#1e293b;border-radius:16px;padding:18px;margin-bottom:14px;border:1px solid #334155;">
            ...
            <div x-show="status === 'dijadwalkan'" x-transition style="display:grid;grid-template-columns:1fr 1fr;gap:14px;background:#0f172a;border-radius:10px;padding:14px;border:1px solid #8b5cf644;">
                <div>
                    ...
                    <input type="date" name="tanggal_jadwal" value="{{ old('tanggal_jadwal', $lead->tanggal_jadwal?->format('Y-m-d')) }}" style="width:100%;background:#1e293b;color:#f1f5f9;border:1px solid #4c1d95;border-radius:8px;padding:9px 12px;font-size:14px;box-sizing:border-box;">
                ...
                <div>
                    ...
                    <input type="time" name="jam_jadwal" value="{{ old('jam_jadwal', $lead->jam_jadwal ? substr($lead->jam_jadwal,0,5) : '') }}" style="width:100%;background:#1e293b;color:#f1f5f9;border:1px solid #4c1d95;border-radius:8px;padding:9px 12px;font-size:14px;box-sizing:border-box;">
        ...
        <div style="background:#1e293b;border-radius:16px;padding:18px;margin-bottom:20px;border:1px solid #334155;">
            ...
            <textarea name="catatan" rows="3" style="width:100%;background:#0f172a;color:#f1f5f9;border:1px solid #334155;border-radius:10px;padding:10px 14px;font-size:14px;resize:vertical;box-sizing:border-box;">{{ old('catatan',$lead->catatan) }}</textarea>
        </div>
